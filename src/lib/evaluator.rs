...
use crate::board_logic::{BoardArr, BoardMarker, Stone};
...
pub fn is_five(board: &BoardArr, marker: &BoardMarker) -> Result<bool, ()> {
...
pub fn is_three(board: &BoardArr, marker: BoardMarker) -> Result<bool, ()> {
    for dir in Direction::iter() {
...
pub fn get_line(board: &BoardArr, marker: &BoardMarker, direction: Direction) -> Result<Line, ()> {
    ...
    match direction {
        Direction::Horizontal => {
            ...
            'right: for i in (marker.point.x + 1)..=board.size() {
                match board.get_xy(i, marker.point.y) {
                    Some(other_marker) => {
                        if other_marker.color == marker.color {
                            line.push((i - marker.point.x) as i8);
                        } else if other_marker.color == marker.color.opposite() {
                            break 'right;
                        }
                    }
                    None => break 'right,
        ...
        Direction::Vertical => {
            let mut line: Line = Line::new(marker, direction);
            'down: for i in (marker.point.y + 1)..=board.size() {
                match board.get_xy(marker.point.x, i) {
                    Some(other_marker) => {
                        if other_marker.color == marker.color {
        ...
        Direction::Diagonal => {
            let mut line: Line = Line::new(marker, direction);
            'diag_down: for i in 1..=board.size() {
                match board.get_xy(marker.point.x + i, marker.point.y + i) {
                    Some(other_marker) => {
                        if other_marker.color == marker.color {
                            line.push(i as i8);
                        } else if other_marker.color == marker.color.opposite() {
        ...
        Direction::AntiDiagonal => {
            let mut line: Line = Line::new(marker, direction);
            'anti_diag_down: for i in 1..=board.size() {
                match board.get_i32xy(
                    (marker.point.x as i32) - (i as i32),
            ...
            'anti_diag_up: for i in 1..=board.size() {
                match board.get_i32xy(
